...
function statusMessage()
{
  var str = 'Kiểu gõ: '
	   if(ON_OFF==0) str +='Tắt'
  else if(TYPMOD==0) str +='Tự động'
  else if(TYPMOD==1) str +='VNI'
  else if(TYPMOD==2) str +='Telex'
  str += (SPELL==1) ? '+Đánh vần': ''
  str += "  ::F8=Đánh vần; F9=Đổi kiểu gõ; F12=Bật/Tắt bộ gõ::";
  status= str
  QBsetCookie()
}
/*******************************/
...
if(typeof(fID)!="undefined" && document.frames[fID])
{
var obj= document.frames[fID];
obj.document.onmousedown= function(){ TXTOBJ=null; FMDown();}
