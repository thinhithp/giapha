...
<script type="text/javascript" src="../Themes/imgslider.js"></script>
  ...
  <tr>
    <td valign="top" width="552">
							<%
								...
								rs3.open "select * from tblnhomanh order by thutu asc", conn 
								if not rs3.eof then
							%>
							<%do while not rs3.eof%>
  <div class="photo-item alpha" style="margin-right:30">
  <div id='basic-modal'>
    <a href="../Xem-anh/?sessionpage=<%=rs3("idnhomanh")%>&/<%=ConvertiTitoloInNomeScript(rs3("name"))%>.html" rel="300" data-fancybox-group="<%=rs3("name")%>" class="photo-img">
      <img width="200" height="154" border="0" src="../<%=rs3("image")%>" alt="<%=rs3("name")%>">
      </img>
      <a class="thuvienanh"><%=rs3("name")%></a>
    </a>
	</div>
  </div>
							<%
