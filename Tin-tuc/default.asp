...
<table cellpadding="0" cellspacing="0" align="center" border="0" width="552">
							<%
								openconn
								set rs= server.CreateObject("adodb.recordset")
								strSQL= "SELECT * FROM tblcat where idmodule=2 order by idcat desc"
								on error resume next
								rs.Open strSQL,Conn,3,1
								If err then Response.Redirect("?tab=1&page="&session("id"))
							...
							<tr>
								<td valign="top">
								<table width="552" height="169" border="0" cellspacing="0" cellpadding="0" background="../images/bgtin.jpg">
								  <tr><td height="15"></td></tr>
								  <tr>
									<td colspan="2" valign="top"><div style="margin-left:20; margin-right:20"><a href="../Tin-chi-tiet/?sessionpage=<%=rs("idcat")%>&/<%=ConvertiTitoloInNomeScript(rs("title"))%>.html" class="tintuc"><b><%=rs("title")%></b></a></div></td>
								  </tr>
								  <tr><td height="4"></td></tr>
								  <tr>
									<td width="136"><div style="margin-left:20"><%if rs("image") <> "" then%><a href="../Tin-chi-tiet/?sessionpage=<%=rs("idcat")%>&/<%=ConvertiTitoloInNomeScript(rs("title"))%>.html"><img src="../<%=rs("image")%>" width="130" height="100"  style="border:1px solid #CCCCCC; padding:2px; margin-left:0px; margin-right:0px" align="left"></a><%end if%></div></td>
									<td valign="top" width="366"><div class="tintuc1" style="margin-right:20"><%=TrimString(rs("quotation"),300)%></td>
								  </tr>
								  <tr><td height="15"></td></tr>
								</table>
								</td>
			...
			<%
					...
					Response.Write "<a class=""xemtrang"" href=""?page=" & intCurrentPage - 1 & """><b>[ Trang tr&#432;&#7899;c ]</b></a>"
					Else
					Response.Write "<font class=""xemtrang"">[ Trang tr&#432;&#7899;c ]</font>"
					End If
					'Print direct access page numbers.
					For intCount = 1 to rs.PageCount	
					If intCount = 1 then
					Response.Write " <font class=""xemtrang"">|</font> "
					End If
					
					Next
					'Print Next Page if required
					If intCurrentPage <> rs.PageCount then
						Response.Write "<a class=""xemtrang"" href=""?page=" & intCurrentPage + 1 & """><b>[ Trang sau ]</b></a>"
					Else
						Response.Write "<font class=""xemtrang"">[ Trang sau ]</font>"
					End If
					End If
