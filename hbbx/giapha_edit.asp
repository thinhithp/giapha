...
<table border="0" cellpadding="7" cellspacing="0" style="border-collapse: collapse" bordercolor="#111111" width="600">
  <tr>
    <td width="600" align="left" class="title1">SỬA THÔNG TIN NHÂN VẬT</td>
  </tr>
  <tr>
    <td width="600">
   	<% 	
	 		 ...
	 		 rs1.open"Select * from tblgiapha where idmodule = '"&request.QueryString("id")&"'",conn
	 %>
	 <table border="0" cellpadding="3" cellspacing="0" style="border-collapse: collapse" width="600">
			...
			<td width="450" align="left">
				<select name="idparrent" style="width:200" class="tintuc1">
									<%
										...
										rs4.open "select * from tblgiapha where idmodule = "&id&"", conn 
										if not rs4.eof then
									%>
									<% if rs4("idparrent")=0 then%>
									<option class="tintuc1" value='0'>&nbsp;&nbsp;-- Thủy tổ - Đời thứ I --</option>
									<% else %>
									<%
										openconn
										set rs5= server.CreateObject("adodb.recordset")
										rs5.open "select * from tblgiapha where idmodule='"&rs4("idparrent")&"' ", conn 
										if not rs5.eof then
									%>
									<option class="tintuc1" value='<%=rs5("idmodule")%>'>&nbsp;&nbsp;-- <%=rs5("name")%> --</option>
									<%
									 end if
									 rs5.close
									 %>
		  ...
		  <tr>
			...
			<td width="450" align="left">
			<input name="title" type="text" class="tintuc1" value="<%=rs1("name")%>" style="width:200"></td>
		  </tr>
		  <tr>
			<td width="150" class="tintuc1">Tên tự (Tên Hiệu/Thụy)</td>
			<td width="450" align="left">
			<input name="tentu" type="text" class="tintuc1" value="<%=rs1("tentu")%>" style="width:200"></td>
		  </tr>
		 <tr>
			...
			<select name="gioitinh" style=" width:200" class="tintuc1">
		  			 ...
		  			 <%
							openconn
							set rs4= server.CreateObject("adodb.recordset")
							rs4.open "select * from tbldoi", conn 
					%>
		  <tr>
			<td width="150" class="tintuc1">Thuộc đời thứ</td>
			<td width="450" align="left">
			<select name="iddoi" style=" width:200" class="tintuc1">
					<%	do while not rs4.eof %>
				<option class="tintuc1" value='<%=rs4("iddoi")%>' <%if rs1("iddoi") = rs4("iddoi") then response.Write("selected")%>>
					<b><%=rs4("name")%></b> - (Đời thứ <%=rs4("positions")%>)
				</option>
				<%
					 rs4.movenext
					 loop
		 ...
		 <tr>
		  ...
		  <tr>
			...
			<td width="450" align="left">
			<input name="nammat" type="text" class="tintuc1" value="<%=rs1("nammat")%>" style="width:200"></td>
		  </tr>
		  <tr>
			<td width="150" class="tintuc1">Hưởng thọ/Hưởng dương</td>
			<td width="450" align="left" class="tintuc1">
			<input name="huongtho" class="tintuc1" type="text" value="<%=rs1("huongtho")%>" style="width:100"> tuổi</td>
		  </tr>
		  <tr>
