...
If Request.Form("btnUpdate")<>"" then
	...
	For i=0 to Ubound(intBlockID)
		...
		strSQL = "UPDATE tblvideo SET thutu=" & CInt(arrBlockOrder(i)) & " WHERE idvideo=" & CInt(intBlockID(i))
		Conn.Execute(strSQL)
	Next
%>
<script language="javascript">
...
<table border="0" cellpadding="7" cellspacing="0" style="border-collapse: collapse" bordercolor="#111111" width="600">
  <tr>
    <td align="left" class="title1">&nbsp;QU&#7842;N L&#221; VIDEO</td>
  </tr>
	<tr>
    ...
    <table border="1" cellpadding="3" cellspacing="0" style="border-collapse: collapse" bordercolor="#B5B5B3" width="600" id="AutoNumber2">
      <tr>
        <td width="400" bgcolor="#DEDBCE" class="title2">&nbsp;TÊN VIDEO</td>
      ...
      <% 	
	  		 openconn	
	  		 set rs=server.CreateObject("adodb.recordset")
	 		 rs.open"Select * from tblvideo order by idvideo DESC",conn,3,1
	 ...
	 %>
	  <tr>
        <td class="tintuc2"><%= rs("name") %>
		<input type="hidden"  class="tintuc2" name="txtBlockID" value="<%=RS("idvideo")%>">
		</td>
		 <td>
		 	<select  class="tintuc2" size="1" name="cbBlockOrder" style="width:70px">
				<%for i=1 to rs.Recordcount%>
				<option  class="tintuc2" value="<%=i%>" <%if rs("thutu")=i then Response.Write "selected"%>><%=i%></option>
				<%Next%>
		</select>
		 </td>
        <td width="50"><div align="center"><a href="?hbxb=28&id=<%=rs("idvideo")%>"><img src="images/sua.png" alt="S?a thng tin" width="24" border="0"></a></div></td>
        <td width="50"><div align="center"><img src="images/xoa.png" alt="Xa thng tin" width="24" style="cursor:hand " onClick="del_one('<%= rs("idvideo") %>');"></div></td>
        </tr>
		<% 
		...
		<tr align="center">
			...
			<%
					...
					Response.Write "<a href=""?hbxb=26&page=" & intCurrentPage - 1 & """ class=""tintuc2"">Trang tr&#432;&#7899;c</a>"
					Else
					...
					Else
						Response.Write "<a href=""?hbxb=26&page=" & intCount & """ class=""tintuc2"">" & intCount & "</a> | "
					End If
					Next
					'Print Next Page if required
					If intCurrentPage <> rs.PageCount then
						Response.Write "<a href=""?hbxb=26&page=" & intCurrentPage + 1 & """ class=""tintuc2"">Trang sau</a>"
					Else
		 ...
		 <tr>
				<td align="center" colspan="6"><input  class="tintuc2" type="button" name="Button" value="Th&#234;m m&#7899;i" onClick="location.href='?hbxb=27&Them-video.html';"> 
