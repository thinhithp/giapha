...
if request("action")="submit" then
	...
	posotion=replace(request("posotion"),"'","''")
	 ...
	 sql=" update tblmodule set idparrent='"&idparrent&"',name=N'"&title&"',positions='"&posotion&"' where idmodule='"&id&"'"
	 conn.execute(sql)
...
<table border="0" cellpadding="7" cellspacing="0" style="border-collapse: collapse" bordercolor="#111111" width="100%">
  <tr>
    <td width="100%" align="center"><b><font size="3" color="#4A865A">S&#7916;A MODULE</font></b></td>
  </tr>
  <tr>
    <td width="100%">
   	<% 	
	 		 ...
	 		 rs1.open"Select * from tblmodule where idmodule = '"&request.QueryString("id")&"'",conn
	 %>
	 <table border="0" cellpadding="3" cellspacing="0" style="border-collapse: collapse" width="100%">
		  ...
		  <%
							...
							rs3.open "select * from tblmodule where idparrent = 0 ", conn 
		  ...
		  <tr>
			...
			<td width="86%">
				<select name="idparrent" style="width:200">
									...
									<option value='<%=rs3("idmodule")%>' <%if rs3("idmodule") = rs1("idparrent") then response.Write("selected")%>><%=rs3("name")%></option>
									<%
										openconn
										set rs4= server.CreateObject("adodb.recordset")
										rs4.open "select * from tblmodule where idparrent='"&rs3("idmodule")&"' ", conn 
									do while not rs4.eof
									%>
									<option value='<%=rs4("idmodule")%>'>&nbsp;&nbsp;- - <%=rs4("name")%></option>
													<%
									 rs4.movenext
									 loop
									 rs4.close
		  ...
		  <tr>
			...
			<td width="86%">
			<input name="title" type="text" value="<%=rs1("name")%>" style="font-family: Verdana; font-size: 8pt; width:200"></td>
		  </tr>
		  <tr>
			<td width="14%" nowrap>Thứ tự</td>
			<td width="86%">
			<input name="posotion" type="text" style="font-family: Verdana; font-size: 8pt; width:200" value="<%=rs1("positions")%>"></td>
		  </tr>
        <tr align="center">
